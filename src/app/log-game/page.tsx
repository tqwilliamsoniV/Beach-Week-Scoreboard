...
export default async function LogGamePage() {
  ...
  const [{ data: games }, { data: players }, { data: variants }] = await Promise.all([
    ...
    supabase.from('game_variants').select('*').order('game_id').order('sort_order'),
  ])

  return (
    <div className="flex flex-col min-h-screen pb-20">
      <LogGameForm games={games ?? []} players={players ?? []} variants={variants ?? []} />
      <BottomNav />
    </div>
  )
}
